...
[![GitHub contributors](https://img.shields.io/github/contributors/pichillilorenzo/flutter_inappwebview_fork)](https://github.com/pichillilorenzo/flutter_inappwebview_fork/graphs/contributors)
[![GitHub forks](https://img.shields.io/github/forks/pichillilorenzo/flutter_inappwebview_fork?style=social)](https://github.com/pichillilorenzo/flutter_inappwebview_fork)
[![GitHub stars](https://img.shields.io/github/stars/pichillilorenzo/flutter_inappwebview_fork?style=social)](https://github.com/pichillilorenzo/flutter_inappwebview_fork)
...
- [Flutter Browser App: A Full-Featured Mobile Browser App (such as the Google Chrome mobile browser) created using Flutter and the features offered by the flutter_inappwebview_fork plugin](https://github.com/pichillilorenzo/flutter_browser_app)
...
Also, you can refer to the [#343](https://github.com/pichillilorenzo/flutter_inappwebview_fork/issues/343) issue.
...
```
=== BUILD TARGET flutter_inappwebview_fork OF PROJECT Pods WITH CONFIGURATION Debug ===
...
If you still have this problem, try to edit iOS `Podfile` like this (see [#15](https://github.com/pichillilorenzo/flutter_inappwebview_fork/issues/15)):
```
target 'Runner' do
...
```xml
<provider
    android:name="com.pichillilorenzo.flutter_inappwebview_fork.InAppWebViewFileProvider"
    android:authorities="${applicationId}.flutter_inappwebview_fork.fileprovider"
...
First, add `flutter_inappwebview_fork` as a [dependency in your pubspec.yaml file](https://flutter.io/using-packages/).
...
See the online [API Reference](https://pub.dartlang.org/documentation/flutter_inappwebview_fork/latest/) to get the full documentation.
...
The JavaScript function that can be used to call the handler is `window.flutter_inappwebview_fork.callHandler(handlerName <String>, ...args)`, where `args` are [rest parameters](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/rest_parameters).
The `args` will be stringified automatically using `JSON.stringify(args)` method and then they will be decoded on the Dart side.

In order to call `window.flutter_inappwebview_fork.callHandler(handlerName <String>, ...args)` properly, you need to wait and listen the JavaScript event `flutterInAppWebViewPlatformReady`.
...
`window.flutter_inappwebview_fork.callHandler` returns a JavaScript [Promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise)
...
<script>
   window.addEventListener("flutterInAppWebViewPlatformReady", function(event) {
     window.flutter_inappwebview_fork.callHandler('handlerFoo').then(function(result) {
       console.log(result);
     });

     window.flutter_inappwebview_fork.callHandler('handlerFooWithArgs', 1, true, ['bar', 5], {foo: 'baz'}).then(function(result) {
       console.log(result);
     });
   });
</script>
...
```dart
  ...
  inAppWebViewController.evaluateJavascript(source: """
    window.flutter_inappwebview_fork.callHandler('test', 'Text from Javascript').then(function(result) {
      console.log(result);
    });
  """);
```
